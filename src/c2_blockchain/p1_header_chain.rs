...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        // todo!("Exercise 3")
        for h in chain.iter(){
            return h.child().height == h.height+1 && h.child().parent == hash(&h.parent)
        }
        false
    }
}
...
fn build_valid_chain_length_5() -> Vec<Header> {
    ...
    for h in 0..=5{
        result_blocks.push(Header { parent: hash(&h), height: h, extrinsics_root: (), state_root: (), consensus_digest: () }) //???
    }
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    let mut result_blocks = Vec::new();
    for h in 0..=5{
        result_blocks.push(Header { parent: h, height: h, extrinsics_root: (), state_root: (), consensus_digest: () })
    }
